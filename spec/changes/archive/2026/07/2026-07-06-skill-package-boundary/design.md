...
- Installed-source updates are part of the same package boundary. A recorded raw `content_url` may identify the package entrypoint for hash comparison, but it must not demote an installed package source into per-file raw HTTP download during update. Package metadata wins over `content_url`; Git/GitHub/Gitea package updates must use the recorded `source_url` plus branch/directory metadata through clone-backed repo sync.
- Repo-to-DB synchronization is a package boundary operation, not an untrusted catalog import. It may trim surrounding whitespace and filter malformed parsed frontmatter field types, but it must preserve complete `SKILL.md` entrypoint text and must not silently truncate content, metadata strings, tags, prerequisites, or compatibility entries. Size limits must be explicit validation failures at an ingress boundary, not mutation during persistence.
