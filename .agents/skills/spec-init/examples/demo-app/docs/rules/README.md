...
- `docs/rules/clarification-rules.md`: 需求澄清、关键疑点和用户确认规则
- `docs/rules/document-routing-rules.md`: 文档语义到目录路径的路由规则

### 实现中

- `docs/rules/coding-standards.md`: 编码规范、注释要求和实现边界
- `docs/rules/testing-standards.md`: 测试分层、回归要求和质量门禁
- `docs/rules/bug-fix-rules.md`: bug 定位、根因修复与回归要求

### 文档同步与变更治理

- `docs/rules/doc-sync-rules.md`: 代码、测试、当前状态文档和长期知识的同步规则
- `docs/rules/change-management-rules.md`: 变更工作区、生命周期和历史记录规则
- `docs/rules/document-archive-rules.md`: 记录型文档编号、索引和按年月归档规则
- `docs/rules/issue-management-rules.md`: issue、风险、技术债和废弃文档管理规则
...
3. 在实现前执行一致性分析，确认需求、设计、验证、任务和变更工作区没有冲突
4. 在实现后执行文档收敛，回写当前文档、长期真相和变更历史
