...
- [ ] T-ANALYZE-001 实现前检查需求、设计、验证、任务和 change workspace 的一致性
  - Requirement: FR-ALL
  - Design: DES-ALL
  - Test: TEST-ALL
  - Output: 一致性缺口清单或“无阻塞项”结论
  - Depends on: T-WEB-002

- [ ] T-CONVERGE-001 实现后回写文档、变更生命周期和长期真相
  - Requirement: FR-ALL
  - Design: DES-ALL
  - Test: TEST-ALL
  - Output: 已同步的 workflow / knowledge / changes / records
  - Depends on: [完成对应实现任务]
...
- 实现前已经完成 analyze，且没有阻塞性缺口
- 实现后已经安排 converge，把真实行为和文档重新对齐
