...
try {
  ...
  $StateRoot = if ($env:PROMPTHUB_DREAM_SKIN_STATE_ROOT) {
    [System.IO.Path]::GetFullPath($env:PROMPTHUB_DREAM_SKIN_STATE_ROOT)
  } else {
    Join-Path $env:LOCALAPPDATA 'PromptHub\CodexDreamSkinRuntime'
  }
