...
export function SkillStoreCard({
  ...
  const badges = storeLabel
    ...
    : variantBadges;
  const cardLabel = skill.description
  ...
  return (
    <div
      ...
      className={`group relative flex items-center gap-3 p-3.5 app-wallpaper-surface border rounded-xl hover:border-primary/40 transition-all animate-in fade-in slide-in-from-bottom-2 hover:shadow-md ${
        isSelected
      ...
      <div className="shrink-0">
        {isInstallingThis ? (
          <button
            type="button"
            disabled
            aria-label={t("skill.installing", "Installing...")}
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              onQuickInstall?.(skill, e);
            }}
            disabled={isInstallingThis}
            aria-label={t("skill.install", "Install")}
